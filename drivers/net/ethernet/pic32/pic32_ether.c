...
#define DRIVER_NAME "pic32-ether"

#define PIC32_SEL(set, reg)	(set ? PIC32_SET(reg) : PIC32_CLR(reg))

static u32 pic32ether_mdc_clk_div(struct pic32ether *bp);
static void pic32ether_reset_hw(struct pic32ether *bp);
static void pic32ether_set_rx_mode(struct net_device *dev);

/* Ring buffer accessors */
static inline unsigned int pic32ether_tx_ring_wrap(unsigned int index)
{
...
pic32ether_tx_dma(struct pic32ether *bp, unsigned int index)
{
	...
	offset = pic32ether_tx_ring_wrap(index) *
		sizeof(struct pic32ether_dma_desc);

	return bp->tx_ring_dma + offset;
}

static inline unsigned int pic32ether_rx_ring_wrap(unsigned int index)
{
...
static void pic32ether_set_hwaddr(struct pic32ether *bp)
{
	u16 val;
...
static void pic32ether_get_hwaddr(struct pic32ether *bp)
{
...
static int pic32ether_mdio_reset(struct mii_bus *bus)
{
	struct pic32ether *bp = bus->priv;
...
static void pic32ether_handle_link_change(struct net_device *dev)
{
	struct pic32ether *bp = netdev_priv(dev);
...
static int pic32ether_mii_probe(struct net_device *dev)
{
	struct pic32ether *bp = netdev_priv(dev);
...
static int pic32ether_mii_init(struct pic32ether *bp)
{
	struct pic32ether_platform_data *pdata;
	...
	bp->mii_bus->read = &pic32ether_mdio_read;
...
static void pic32ether_update_stats(struct pic32ether *bp)
{
...
static int pic32ether_halt_tx(struct pic32ether *bp)
{
...
static int pic32ether_poll(struct napi_struct *napi, int budget)
{
	struct pic32ether *bp = container_of(napi, struct pic32ether, napi);
	int work_done;
	u32 status;
...
static irqreturn_t pic32ether_interrupt(int irq, void *dev_id)
{
...
static int pic32ether_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	struct pic32ether *bp = netdev_priv(dev);
	dma_addr_t mapping;
...
static void pic32ether_init_rx_buffer_size(struct pic32ether *bp)
{
...
static int pic32ether_alloc_rings(struct pic32ether *bp)
{
	int size;
...
static void pic32ether_init_rings(struct pic32ether *bp)
{
	int i;
	dma_addr_t addr;
	struct pic32ether_dma_desc *desc;
...
static void pic32ether_init_hw(struct pic32ether *bp)
{
	...
	mac_writel(bp, EMAC1CFG2, MAC_BIT(EMAC1CFG2_EXCESSDFR) |
		...
		MAC_BIT(EMAC1CFG2_NOBKOFF) |
		MAC_BIT(EMAC1CFG2_LENGTHCK));
...
static int pic32ether_open(struct net_device *dev)
{
	struct pic32ether *bp = netdev_priv(dev);
	int err;
	...
	pic32ether_init_rx_buffer_size(bp);

	err = pic32ether_alloc_rings(bp);
	if (err) {
...
static struct net_device_stats *pic32ether_get_stats(struct net_device *dev)
{
	struct pic32ether *bp = netdev_priv(dev);
	struct net_device_stats *nstat = &bp->stats;
	struct pic32ether_stats *hwstat = &bp->hw_stats;

	/* read stats from hardware */
	pic32ether_update_stats(bp);
...
static int pic32ether_get_regs_len(struct net_device *netdev)
{
	return 9 * sizeof(u32);
}

static void pic32ether_get_regs(struct net_device *dev,
				...
				void *p)
{
	...
	tail = pic32ether_tx_ring_wrap(bp->tx_tail);
	head = pic32ether_tx_ring_wrap(bp->tx_head);
	...
	regs_buff[5]  = tail;
	regs_buff[6]  = head;
	regs_buff[7] = pic32ether_tx_dma(bp, tail);
	regs_buff[8] = pic32ether_tx_dma(bp, head);
}

static const struct ethtool_ops pic32ether_ethtool_ops = {
	.get_settings		= pic32ether_get_settings,
	.set_settings		= pic32ether_set_settings,
	.get_regs_len		= pic32ether_get_regs_len,
	.get_regs		= pic32ether_get_regs,
	.get_link		= ethtool_op_get_link,
	.get_ts_info		= ethtool_op_get_ts_info,
};

static int pic32ether_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	struct pic32ether *bp = netdev_priv(dev);
	struct phy_device *phydev = bp->phy_dev;
...
static const struct net_device_ops pic32ether_netdev_ops = {
	.ndo_open		= pic32ether_open,
	.ndo_stop		= pic32ether_close,
	.ndo_start_xmit		= pic32ether_start_xmit,
	.ndo_set_rx_mode	= pic32ether_set_rx_mode,
	.ndo_get_stats		= pic32ether_get_stats,
	.ndo_do_ioctl		= pic32ether_ioctl,
...
static int __exit pic32ether_remove(struct platform_device *pdev)
{
	struct net_device *dev;
	struct pic32ether *bp;
...
static struct platform_driver pic32ether_driver = {
	.remove		= __exit_p(pic32ether_remove),
	.driver		= {
		.name	= DRIVER_NAME,
		.owner	= THIS_MODULE,
