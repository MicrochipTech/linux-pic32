...
static u32 pic32_conf_set_clr_reg(u32 offset, u32 clr, u32 set)
{
	u32 v;
...
{
	return pic32_conf_set_clr_reg(PIC32_CFGCON2, 0, BIT(31));
}

int pic32_disable_lcd(void)
{
	return pic32_conf_set_clr_reg(PIC32_CFGCON2, BIT(31), 0);
}
...
int pic32_set_lcd_mode(int mode)
{
	...
	return pic32_conf_set_clr_reg(PIC32_CFGCON2, BIT(30), mask);
}

void pic32_enable_ebi_addr_pins(int mask)
{
	pic32_conf_set_clr_reg(PIC32_CFGEBIA, ~0, mask);
}

void pic32_enable_ebi_ctrl_pins(int mask)
{
	pic32_conf_set_clr_reg(PIC32_CFGEBIC, ~0, mask);
}
...
int pic32_set_sdhc_adma_fifo_threshold(u32 rthrsh, u32 wthrsh)
{
	...
	return pic32_conf_set_clr_reg(PIC32_CFGCON2, clr, set);
}
...
{
	pic32_conf_set_clr_reg(PIC32_CFGCON, 0x3 << 4, mode << 4);
}
